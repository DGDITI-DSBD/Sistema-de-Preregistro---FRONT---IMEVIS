...
class oRegController extends Controller
{
    ...
    public function updateStatePreRegAccion(Request $request){
        ...
        log::info('entrando datos', array($getForm));

       $responseFunction =  $this -> RulesOfGeneralData($getForm);

       log::info('validando datos', array($responseFunction));
       ...
       if(is_array($responseFunction) && array_key_exists('success', $responseFunction)){
            $getFlag = $responseFunction['success'];
                if(!$getFlag){
                    ...
                    $this -> curp                    = $getForm['curp'];//se agrego campo por validaciones
                    $this -> id_nacionalidad         = $getForm['id_nacionalidad'];//se agrego campo por validaciones 
                    ...
                    log::info('Estado del folio', array($getEstadoFolio));
                        ...
                        if(!empty($getEstadoFolio) || !is_null($getEstadoFolio) ){
                            ...
                            try{
                                ...
                                log::info('Fila lista', array($getFilaLista));
                                ...
                                if($getFilaLista <= 0 || $getFilaLista > 1){
                                    ...
                                    return response() -> json([
                                        ...
                                        'data'    => []
                                            ...
                                            $getFilaPrereg = si_prereg::where('si_ctrl_lista_id', $this -> folio_asignado)
                                            ->update([
                                                'telefono'               => $this -> telefono,
                                                'telefono_fijo'          => $this -> telefono_fijo,
                                                'ct_edo_civil'           => $this -> ct_edo_civil,
                                                'cve_tipo_id_ofl'       => $this -> cve_tipo_id_ofcl,
                                                'id_docto_ofl'           => $this -> id_docto_ofl,
                                                'email'                  => $this -> e_mail
                                            ]);

                                            log::info('Fila pre registro', array($getFilaPrereg));
                                ...
                                return response() -> json([
                                    'success' => false,
                                    'message' => '50007',
                                    'error'   => 'Actualización de datos'.$error,
                                    'data'    => []
                                ], 500);
                            }
       ...
       if(is_array($responseFunction) && array_key_exists('success', $responseFunction)){
            ...
            $getFlag = $responseFunction['success'];
                if(!$getFlag){
                    ...
                    $getEstadoFolio = si_ctrlLista::select('estado_registro')->where('curp', $getForm['curp'])->first();

                    Log::info("Estado Folio", array($getEstadoFolio));
                        ...
                        if(!empty($getEstadoFolio) || !is_null($getEstadoFolio) ){
                            ...
                            try{
                                ...
                                $getFilaPrereg = si_prereg::where('si_ctrl_lista_id', $this -> id)
                                ->update([
                                    'calle'                  => $this -> calle,
                                ...
                                Log::info("Estado del Registro al actualizar", array($getFilaPrereg));
