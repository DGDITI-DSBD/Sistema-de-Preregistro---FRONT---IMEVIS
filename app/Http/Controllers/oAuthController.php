...
class oAuthController extends Controller{
    ...
    // *****************************************
        ...
        public function index(Request $request){
            ...
            $data = $request -> data;
            log::info('LLEGADA DE LOS DATOS', array($data));
                ...
                if(!empty($data) || !is_null($data)){
                    ...
                    $info = si_ctrlLista::with('preregistro:id,cve_tipo_id_ofl,id_docto_ofl,ct_edo_civil,calle,num_ext,num_int,entre_calle,y_calle,otra_referencia,cve_asentamiento,colonia,cve_localidad,localidad,cve_municipio,municipio,cve_entidad_federativa,codigo_postal,telefono,telefono_fijo,email,fecha_registro,fecha_solicitud')
                                        ...
                                        ->selectRaw('curp,TRIM(sexo) AS sexo,primer_apellido,segundo_apellido,nombre,cve_entidad_nacimiento,cve_nacionalidad,fecha_nacimiento,edad,estado_registro,id')
                                        ->where('id',$data['id'])
                                        // ->where('curp',$data['curp'])
                                        ->first();

                    // $info = si_ctrlLista::find($data['id'])->with('preregistro:id')->with('estudio:id')->first();
                    log::info('DATOS ENCONTRADOS', array($info));
    ...
    public function controlTokenFolio(Request $request){
        ...
        $state = 0;
            ...
            Log::info("Entrando datos ", array($info));
                        ...
                        $lock = Cache::lock('MB_'.$getCurpState, 5);
                            if($lock -> get()){
                                ...
                                try{
                                    ...
                                    Log::info("Generando Folio ", array($lock));
    

                                    $getFolio = $this -> controlAssignFolio();

                                                                    Log::info("Folio obtenido ", array($getFolio));
                                    ...
                                    $getStateReg = $this -> verifyingCURP(
                                        ...
                                        Log::info("Iniciando Registro", array($getStateReg));
    ...
    public function verifyingCURP($si_folio, 
        ...
        try {
            ...
            $getEntidadNac = si_cat_entidad_federativa::select("entidadfedid", "entidadfed_desc", "entidafed_abrev")
                ->where("entidafed_abrev", $cveEntidadNac)
                ->first();
                log::info('entidad', array($getEntidadNac));

            $setNacionId = $getNacionId -> cve_pais;
            $setEntidadNac = $getEntidadNac -> entidadfedid;
            ...
            $newOnLista -> estado_verificado      = 0;
            $newOnLista -> estado_termino         = 2;
