...
import { cleaningCurp, responseCurp, sendingCurp, cleaningFile, responseFile, sendingFile } from '../store';
import { HiPhoneOutgoing, HiPencilAlt, HiHeart, HiShieldExclamation, HiShieldCheck } from "react-icons/hi";
import useWindowSize from '../hooks/useWindowSize';


export const LandingRouterSimple = () => {
  const [folioParam, setFolioParam] = useState(0);
  const [curpParam, setCurpParam] = useState(0);
  let token;

  const params = useParams();
  useEffect(() => {
    if( params?.folio && params?.folio !== undefined ){
      var base64regex = /^([0-9a-zA-Z+/]{4})*(([0-9a-zA-Z+/]{2}==)|([0-9a-zA-Z+/]{3}=))?$/;
      base64regex.test(params.folio) ? setFolioParam(atob(params.folio)) : null;
    }
    if( params?.curp && params?.curp !== undefined ){
      var base64regex = /^([0-9a-zA-Z+/]{4})*(([0-9a-zA-Z+/]{2}==)|([0-9a-zA-Z+/]{3}=))?$/;
      base64regex.test(params.curp) ? setCurpParam(atob(params.curp)) : null;
    }
  }, [token]);
  ...
  const [validcurp, setValidCurp] = useState(2);
  const [pasoUno, setPasoUno] = useState(null);
  const [pasoDos, setPasoDos] = useState(null);
  const [captchaValue, setCaptchaValue] = useState(true);
  const { loadingCurp } = useSelector(state => state.curp);
  const { loadingFile } = useSelector(state => state.file);
  const [persona, setPersona] = useState('');
  const [searchBarQuery, setSearchBarQuery] = React.useState([""])

  useEffect(() => {
    setCurp(folioParam, curpParam);
  }, [folioParam, curpParam]);
  ...
  const setCurp = (folio, curp) => {
    console.log(folio);
    dispatch(sendingCurp());
    sendInfo(folio, curp);
  }

  useEffect(() => {
    if(pasoUno == 1 && pasoDos == 1 )
      setValidCurp(1);
    else
      setValidCurp(2);
  }, [pasoUno, pasoDos]);
  ...
  const sendInfo = async (cfo, cid) => {
     await fetch('http://10.40.140.134:8008/api/v1/get-info/' + cid, {
     //  await fetch('https://ddsisem.edomex.gob.mx/btdis-backend/api/v1/get-info/'+cfo+'/'+cid, {
      method: 'GET',
      //  body: JSON.stringify({
      //     data: {curp: cid}
      //  }),
      ...
      .then(jsondata => {
        ...
        if (jsondata.success && jsondata.data != null) {
          ...
          setRespuesta(jsondata.success);
        } else {
          setMensaje(jsondata.success);
          setRespuesta(666);
        }

        // if(jsondata.message == 'No está validado')
        // {
        //   setMensaje(false);
        //   setRespuesta(555);
        // }

        // if(jsondata.message == 'Failed')
        // {
        //   setMensaje(false);
        //   setRespuesta(333);
        // }

        // if(jsondata?.case == 'Error de conexión')
        // {
        //   setMensaje(false);
        //   setRespuesta(444);
        // }

        jsondata.success == true ? setPersona(jsondata.data) : setPersona({ estatus: false });
        //setPersona(jsondata.data);
        dispatch(responseCurp());
        //console.log(respuesta);
      })
  };

  const sendFile = async (fid) => {
     await fetch('http://10.40.140.134:8008/api/v1/get-file/', {
    // await fetch('https://ddsisem.edomex.gob.mx/btdis-backend/api/v1/get-file', {
      method: 'POST',
      body: JSON.stringify({
        data: { folio: fid }
      }),
      ...
      .then(jsondata => {
        ...
        if (jsondata.pdf) {
          ...
          var attachmentName = "Certificado_de_Reconstruccion_Mamaria.pdf";
  ...
  const handleClick = event => {
    event.preventDefault();
     console.log(message.length > 0 , messageCurp.length > 0);
    if (message.length > 0 && messageCurp.length > 0) {
      setCurp(message, messageCurp);
      // setCurp(message)
    }
  };
  ...
  const handleClickFile = event => {
    ...
    sendFile(persona?.folio);
  };

  const handleChange = event => {
    setMessage(event.target.value.toUpperCase());
    if (event.target.value.match("^([A-Z]{4})([0-9]{5})$") != null){
        setPasoUno(1);
      }else{
        setPasoUno(null);
    }   
    ...
    const handleChangeCurp = event => {
      setMessageCurp(event.target.value.toUpperCase());
      if(event.target.value.match("^([a-z,A-Z]{4})([0-9]{6})([a-z,A-Z]{6})([0-9,a-z,A-Z]{2})$")!=null){
          setPasoDos(1);
      }else{
          setPasoDos(null);
    }
    ...
    useEffect(() => {
      const segundaSeccion = document.getElementById('checker');
  ...
  return (
    ...
    <div className={`relative bg-white `} >
      <div className="max-w-7xl md:mx-64 mx-auto">
        <div
          className={`relative z-10 pb-2 bg-white sm:pb-2 md:pb-2 lg:max-w-2xl lg:w-full lg:pb-2 xl:pb-2`}
        >
          {/* <h1>-{folioParam + '-' + curpParam}-</h1> */}
          <Header />
          <MainHero />
        </div>
      </div>
    ...
    <LazyShow>
      ...
      <div className={`bg-white mt-10 mb-10`} id="checker">
      {/* <form onSubmit={handleSubmit}>  */}
      {/* <form onSubmit={ handleSubmit(onSubmit) }> */}
      <section className="h-full bg-white items-center">
        <div className={`${size.width <= 400 ? "pl-2 pr-4" : "pl-8 pr-8"}`}>
                  ...
                  <div className="fixed top-0 left-0 z-50 w-screen h-screen flex items-center justify-center" style={{ background: "rgba(0, 0, 0, 0.3)" }} >
                    <div className="bg-white border py-2 px-5 rounded-lg flex items-center flex-col">
                      <div className="loader-dots block relative w-20 h-5 mt-2">
                        <div className="absolute top-0 mt-1 w-3 h-3 rounded-full bg-[#FA6FB8]"></div>
                        <div className="absolute top-0 mt-1 w-3 h-3 rounded-full bg-[#FA6FB8]"></div>
                        <div className="absolute top-0 mt-1 w-3 h-3 rounded-full bg-[#FA6FB8]"></div>
                        <div className="absolute top-0 mt-1 w-3 h-3 rounded-full bg-[#FA6FB8]"></div>
                      </div>
                  ...
                  <div className="px-4 md:px-0 lg:w-6/12">
                    <div className="md:mx-3 md:p-1">
                      ...
                      <img
                        ...
                        src="https://ddsisem.edomex.gob.mx/sicur/images/auth-decoration.svg"
                        alt="logo"
                      />
                      ...
                      <div className='col-span-6 md:pt-12'>
                        <h1 className={`text-2xl font-bold text-center text-[#F43D99] col-span-6 `}>¡Consulta el estatus de tu solicitud!</h1>
                      ...
                      {mensaje.length == 0 ?
                        <div className="grid grid-cols-6 gap-6">
                          ...
                          <div className={`col-span-6 gap-6 ${size.width >= 800 ? "flex" : ""} `} style={{ justifySelf: "center" }}>
                            <div className="col-span-6">
                              <div className="text-center block">
                                <Label
                                  htmlFor="foliol"
                                  value="Ingresa el Folio emitido por la plataforma"
                                />
                              </div>
                              <label htmlFor="website-admin" className="block mb-2 text-sm font-medium text-gray-900"></label>
                              <div className="flex p-6">
                                <span className="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-r-0 border-gray-300 rounded-l-md">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="w-6 h-6 bi bi-123" viewBox="0 0 16 16">
                                ...
                                <input
                                  ...
                                  maxLength={9}
                                  autoComplete="off"
                                  className={`rounded-none rounded-r-lg bg-gray-50 border text-gray-900 ${pasoUno === 1 && message.length > 0 ? "focus:ring-emerald-500" : "focus:ring-red-500"} ${pasoUno === 1 && message.length > 0 ? "focus:border-emerald-500" : "focus:border-red-500"} block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5`}
                            ...
                            <div className="col-span-6">
                              ...
                              <div className="flex p-6">
                                ...
                                <input
                                  ...
                                  maxLength={18}
                                  autoComplete="off"
                                  className={`rounded-none rounded-r-lg bg-gray-50 border text-gray-900 ${pasoDos === 1 && messageCurp.length > 0 ? "focus:ring-emerald-500" : "focus:ring-red-500"} ${pasoDos === 1 && messageCurp.length > 0 ? "focus:border-emerald-500" : "focus:border-red-500"} block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5`}
                        ...
                        <div className="col-span-6 gap-6">
                          {message.length == 0 ? <label htmlFor="website-admin" className="text-center  block mb-2 text-sm font-medium text-red-500">Información requerida</label> : null}
                            {validcurp == 0 && message.length > 0 ? <label htmlFor="website-admin" className="text-center block mb-2 text-sm font-medium text-red-500">Formato de folio no valido</label> : null}
                            {validcurp == 1 ? <label htmlFor="website-admin" className="text-center block mb-2 text-sm font-medium text-emerald-500">¡Correcto!</label> : null}
                        </div>

                          {validcurp == 1
                            ?
                            <div className="col-span-6 justify-center" style={{ justifySelf: "center" }} >
                              <button
                                onClick={handleClick}
                                disabled={loadingCurp == 'enviando' ? true : (loadingCurp == 'recibido' ? true : false)}
                                style={{
                                  justifySelf: "center",
                                  background:
                                    //"linear-gradient(to right, #050708, #0F1619, #0F1519, #222B2F)",
                                    "linear-gradient(to right, #E74694,  #9061F9)",
                                }}
                                type="button" className={` over:bg-[#050708]/90" text-white bg-[#050708] focus:ring-2 focus:outline-none focus:ring-[#E74694]/50 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-[#050708]/50" mr-2 mb-2 mt-4`}>
                                {loadingCurp == 'esperando' ?
                                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-search mr-2" viewBox="0 0 16 16">
                                    ...
                                    </svg> : <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-search mr-2" viewBox="0 0 16 16">
                                      <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z" />
                                    </svg>)}
                                Buscar
                              </button></div> : null}
                          {/* <ReCAPTCHA onChange={onChange} className='mt-6' 
                sitekey="6LcLNpgoAAAAAL9ij7VCOayiA0yMAhb6fq_1I1cl" //preproduccion
                  // sitekey="6LfjdKwoAAAAAEHFNfZ4Iv0f0zpyIeR21uqXDoLj" 
                  //produccion
               /> */}
                          {/* {captchaValue == null ? <label htmlFor="website-admin" className="block mb-2 text-sm font-medium text-red-500">Información requerida</label> : null } */}
                        </div> :
                        (respuesta == true ?
                          <div className="overflow-x-auto mb-1 mt-6">
                            <Card className="max-w ml-5 mb-5 mr-5">
            ...
            <Dropdown.Item icon={HiCurrencyDollar}>Ocultar</Dropdown.Item>
                              ...
                              <div className="text-center items-center ">
                                <div className="text-center items-center ">
                                  <div className="mb-6" style={{ textAlign: "-webkit-center" }}>
                                    <img
                                      className="w-28 h-28 mb-1 rounded-full shadow-lg"
                                      src={`${persona?.sexo == 'Masculino' ?
                                        "https://ddsisem.edomex.gob.mx/sigeco/img/male.jpg" : "https://ddsisem.edomex.gob.mx/sigeco/img/female.jpg"}
                        `}
                                      alt="enterprise"
                                      style={{ objectFit: "cover" }}
                                    />
                                  </div>
                                  <div className="grid pb-1 pl-4">
                                    ...
                                    <div className="" style={{ justifySelf: "center" }}>
                                      {persona?.status == 0 ? (
                                        <div className="flex">
                                          <HiPhoneOutgoing className="w-5 h-5  text-sky-400" />
                                          &nbsp;&nbsp;
                                      ...
                                      {persona?.status == 1 ? (
                                        <div className="flex">
                                          <HiPencilAlt className="w-5 h-5  text-sky-700" />
                                          &nbsp;&nbsp;
                                      ...
                                      {persona?.status == 2 ? (
                                        <div className="flex">
                                          <HiHeart className="w-5 h-5  text-rose-300" />
                                          &nbsp;&nbsp;
                                    ...
                                    {persona?.status == 0 ? (
                                      <span className="text-sm font-semibold text-gray-400 -mt-2 mb-4">
                                        Su solciticud se encuentra en espera de confirmar la cita con la unidad.
                                      </span>) : null}
                                    {persona?.status == 1 ? (
                                      <span className="text-sm font-semibold text-gray-400 -mt-2 mb-4">
                                        Su solciticud fue aceptada y esta en espera de que acuda a la cita para su valoración.
                                      </span>) : null}
                                    {persona?.status == 2 ? (
                                      <span className="text-sm font-semibold text-gray-400 -mt-2 mb-4">
                                        Debe acudir a la unidad donde se le realizara la cirugía.
                                      </span>) : null}
                                    {persona?.status == 3 ? (
                                      <span className="text-sm font-semibold text-gray-400 -mt-2 mb-4">
                                    ...
                                    {persona?.status == 4 ? (
                                      <span className="text-sm font-semibold text-gray-400 -mt-2 mb-4">
                                        No acudiste a tu cita, por lo que debes iniciar desde el proceso de registro.
                                      </span>) : null}
                                    {persona?.status == 5 ? (
                                      <span className="text-sm font-semibold text-gray-400 -mt-2 mb-4">
                                        Su solciticud no se puede llevar a cabo debido a que no cumple con los criterios solcicitados.
                                      </span>) : null}
                                    {persona?.status == 6 ? (
                                      <span className="text-sm font-semibold text-gray-400 -mt-2 mb-4">
                                        Por favor, mantente al pendiente de tu teléfono y/o correo electrónico, ya que el Personal de Salud se pondrá en contacto contigo en breve para confirmar cita y darte la información necesaria.
                                      </span>) : null}
                                    {persona?.status == 7 ? (
                                      <span className="text-sm font-semibold text-gray-400 -mt-2 mb-4">
                                        Nuestro equipo médico te ha evaluado cuidadosamente, y han determinado que, no es viable debido a:  <br></br><b>{persona?.motivo}</b>
                                      </span>) : null}
                                    {/* 5: Rechazado */}
                                    ...
                                    {persona?.status <= 1 && persona?.unidadcita != null ?
                                      <div className="mt-4 grid space-x-3 lg:mt-6">
                                        <span className="text-lg text-colorTerciario font-semibold grid ">
                                          Datos de su cita
                                          {persona?.status == 0 ?
                                            <small className="text-xs text-slate-400 font-semibold ">(En espera de confirmación)</small> : null}
                                        </span>
                                        <span className="text-sm text-gray-950 font-semibold mt-4 ">
                                          Fecha:&nbsp;&nbsp;
                                          <small className="text-sm text-gray-500 font-normal ">
                                            {new Date(persona?.fechacita + ' 00:00:00').toLocaleDateString(
                                              "es-es",
                                              {
                                                weekday: "long",
                                                year: "numeric",
                                        ...
                                        <span className="text-sm text-gray-950 font-semibold ">
                                          Horario:&nbsp;&nbsp;
                                          <small className="text-sm text-gray-500 font-normal ">
                                            {persona?.hora == null ? 'Sin asignar' : persona?.hora}
                                          </small>
                                        </span>

                                      </div>
                                      : (persona?.status >= 2 ? null :
                                        <div className="mt-4 grid space-x-3 lg:mt-6">
                                          <span className="text-lg text-colorTerciario font-semibold grid ">
                                            Sin cita asignada
                                          </span>
                                          <small className="text-xs text-slate-400 font-semibold ">(En espera de fechas disponibles)</small>
                                        </div>)}

                                    {/*  */}
                                    {/* {persona?.status == 3 ?
                                      <div className="justify-center w-52 justify-self-center flex mt-6 cursor-pointer mb-3 rounded-lg pb-2 pt-2.5 text-xs  leading-normal text-white shadow-[0_4px_9px_-4px_rgba(0,0,0,0.2)] transition duration-150 ease-in-out hover:shadow-[0_8px_9px_-4px_rgba(0,0,0,0.1),0_4px_18px_0_rgba(0,0,0,0.2)] focus:shadow-[0_8px_9px_-4px_rgba(0,0,0,0.1),0_4px_18px_0_rgba(0,0,0,0.2)] focus:outline-none focus:ring-0 active:shadow-[0_8px_9px_-4px_rgba(0,0,0,0.1),0_4px_18px_0_rgba(0,0,0,0.2)]"
                                        style={{
                                          background:
                                            "linear-gradient(to right, #000000,  #000000)",
                                        }}>
                                        {loadingFile == 'esperando' ?
                                          <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-cloud-download w-5 h-5 text-white stroke-2" viewBox="0 0 16 16">
                                            <path d="M4.406 1.342A5.53 5.53 0 0 1 8 0c2.69 0 4.923 2 5.166 4.579C14.758 4.804 16 6.137 16 7.773 16 9.569 14.502 11 12.687 11H10a.5.5 0 0 1 0-1h2.688C13.979 10 15 8.988 15 7.773c0-1.216-1.02-2.228-2.313-2.228h-.5v-.5C12.188 2.825 10.328 1 8 1a4.53 4.53 0 0 0-2.941 1.1c-.757.652-1.153 1.438-1.153 2.055v.448l-.445.049C2.064 4.805 1 5.952 1 7.318 1 8.785 2.23 10 3.781 10H6a.5.5 0 0 1 0 1H3.781C1.708 11 0 9.366 0 7.318c0-1.763 1.266-3.223 2.942-3.593.143-.863.698-1.723 1.464-2.383" />
                                            <path d="M7.646 15.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 14.293V5.5a.5.5 0 0 0-1 0v8.793l-2.146-2.147a.5.5 0 0 0-.708.708z" />
                                          </svg> : (loadingFile == 'enviando' ?
                                            <svg aria-hidden="true" role="status" className="inline w-4 h-4 text-gray-200 animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                              <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                                              <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="#1C64F2" />
                                            </svg> : <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-cloud-download w-5 h-5 text-white stroke-2" viewBox="0 0 16 16">
                                              <path d="M4.406 1.342A5.53 5.53 0 0 1 8 0c2.69 0 4.923 2 5.166 4.579C14.758 4.804 16 6.137 16 7.773 16 9.569 14.502 11 12.687 11H10a.5.5 0 0 1 0-1h2.688C13.979 10 15 8.988 15 7.773c0-1.216-1.02-2.228-2.313-2.228h-.5v-.5C12.188 2.825 10.328 1 8 1a4.53 4.53 0 0 0-2.941 1.1c-.757.652-1.153 1.438-1.153 2.055v.448l-.445.049C2.064 4.805 1 5.952 1 7.318 1 8.785 2.23 10 3.781 10H6a.5.5 0 0 1 0 1H3.781C1.708 11 0 9.366 0 7.318c0-1.763 1.266-3.223 2.942-3.593.143-.863.698-1.723 1.464-2.383" />
                                              <path d="M7.646 15.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 14.293V5.5a.5.5 0 0 0-1 0v8.793l-2.146-2.147a.5.5 0 0 0-.708.708z" />
                                            </svg>)}
                                        <button className="ml-2 text-white font-semibold"
                                          onClick={handleClickFile}
                                          disabled={loadingFile == 'enviando' ? true : (loadingFile == 'recibido' ? true : false)}
                                        >{loadingFile == 'esperando' ? "Descarga tu comprobante" : (loadingFile == 'enviando' ? "" : "Descarga tu comprobante")} </button>
                                      </div> : null} */}
                                    ...
                                    <button onClick={(e) => setDefault()}
                                      className="mt-6 cursor-pointer mb-3 inline-block w-full rounded px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_rgba(0,0,0,0.2)] transition duration-150 ease-in-out hover:shadow-[0_8px_9px_-4px_rgba(0,0,0,0.1),0_4px_18px_0_rgba(0,0,0,0.2)] focus:shadow-[0_8px_9px_-4px_rgba(0,0,0,0.1),0_4px_18px_0_rgba(0,0,0,0.2)] focus:outline-none focus:ring-0 active:shadow-[0_8px_9px_-4px_rgba(0,0,0,0.1),0_4px_18px_0_rgba(0,0,0,0.2)]"
                                      style={{
                                        background:
                                          "linear-gradient(to right, #E74694,  #9061F9)",
                                      }}
                          ...
                          <div className="text-2xl font-bold text-center col-span-6 overflow-x-auto mb-1 mt-1 p-10">
                            <section>
                              <div className="bg-white text-white">
                                <div className="h-3/4 flex rounded-lg border border-gray-200 bg-white shadow-md dark:border-gray-700 dark:bg-gray-800 flex-col max-w ml-5 mb-5 mr-5">
                                  <div className="m-auto text-center p-6">
                                    ...
                                    {/* <p className="text-sm md:text-base text-[#9061F9] p-2 mb-4">¡Lo sentimos!</p> */}
                                    <h1 className={`pb-2 pt-1 text-sm md:text-base font-bold text-gray-600 mt-6`}>¡Lo sentimos!</h1>
                                    <h2 className={`pb-2 pt-1 text-sm md:text-base font-bold text-gray-600`}>No se encontró un registro de solicitud con el folio solicitado.</h2>
                                    <h2 className={`pb-2 pt-1 text-sm md:text-base font-bold text-gray-600 mb-10`}>Inténtelo de nuevo más tarde.</h2>
                                    <a onClick={(e) => setDefault()}
                                      style={{
                                        background:
                                          "linear-gradient(to right, #E74694,  #9061F9)",
                                      }}
                      ...
                      {/* <div className="mb-1 pb-1 pt-1 text-center">
                        <button disabled={ isSaving } type="submit" 
                         ...
                         style={{
                           background:
                             "linear-gradient(to right, #BC955B, #BC955B, #efe1ca, #efe1ca)",
                         }}
                        >
                  ...
                  >
                    <div className="relative lg:inset-y-0 lg:right-0 lg:w-full" style={{ height: "900px" }}>
                      <img alt="logo" className="" src={"https://ddsisem.edomex.gob.mx/sigeco/img/people-2569404_1280.jpg"} style={{ objectFit: "cover", width: "100%", height: "-webkit-fill-available" }} />
